	...
	Trace& ss)		///< Trace file to output to
{
	...
	if (output_downloaded_rtcm_json){	ss << "\tdownloaded_rtcm_json_filename: " << downloaded_rtcm_json_filename	<< "\n"; }
	if (output_uploaded_rtcm_json)	{	ss << "\tuploaded_rtcm_json_filename:   " << uploaded_rtcm_json_filename	<< "\n"; }
	...
	boost::program_options::variables_map&	newCommandOpts)	///< Variable map object of command line options
{
	...
	auto output_files = stringsToYamlObject({yaml, ""}, 		{"output_files"});
	{
		...
		trySetFromYaml(output_downloaded_rtcm_json,		output_files, {"output_downloaded_rtcm_json"	},	"(bool) Enable exporting downloaded RTCM data to file");
		trySetFromYaml(downloaded_rtcm_json_directory,	output_files, {"downloaded_rtcm_json_directory"	},	"(string) Directory to export downloaded RTCM data");
		trySetFromYaml(downloaded_rtcm_json_filename,	output_files, {"downloaded_rtcm_json_filename"	},	"(string) Downloaded RTCM data filename");

		trySetFromYaml(output_uploaded_rtcm_json,		output_files, {"output_uploaded_rtcm_json"		},	"(bool) Enable exporting uploaded RTCM data to file");
		trySetFromYaml(uploaded_rtcm_json_directory,	output_files, {"uploaded_rtcm_json_directory"	},	"(string) Directory to export uploaded RTCM data");
		trySetFromYaml(uploaded_rtcm_json_filename,		output_files, {"uploaded_rtcm_json_filename"	},	"(string) Uploaded RTCM data filename");
	...
	tryAddRootToPath(root_output_dir,				downloaded_rtcm_json_directory);
	tryAddRootToPath(root_output_dir,				uploaded_rtcm_json_directory);
	...
	tryAddRootToPath(downloaded_rtcm_json_directory,downloaded_rtcm_json_filename);
	tryAddRootToPath(uploaded_rtcm_json_directory,	uploaded_rtcm_json_filename);
	...
	replaceTags(uploaded_rtcm_json_directory);
	replaceTags(downloaded_rtcm_json_directory);
	...
	replaceTags(uploaded_rtcm_json_filename);
	replaceTags(downloaded_rtcm_json_filename);
	...
	for (auto nav : {false, true})
	{
		...
		for (auto streamUrl : *streamNode_ptr)
		{
			...
			string hostname = fullUrl.substr(0,				slashPos + 1);		// e.g. http://user:pass@auscors.ga.gov.au:2101/BCEP00BKG0
		...
		for (auto& [id, streamUrl] : ntripStreams)
		{
			...
			string mount = streamUrl.substr(streamUrl.find_last_of("/") + 1);
			...
			//prepare to download streams if enabled
			{
				...
				string filename;
				if (nav)		filename = rtcm_nav_filename;
				else			filename = rtcm_obs_filename;

				bool createFile = false;
				if (nav == true		&& record_rtcm_nav)		createFile = true;
				if (nav == false	&& record_rtcm_obs)		createFile = true;
