...
#	else
	BOOST_LOG_TRIVIAL(warning) << "Warning: SSR exporting to JSON requires compilation with MongoDB.";
#	endif
}
	...
	unsigned int	message_length)
{
	...
	for (int sat = 0; sat < numSats; sat++)
	{
			...
			||message_number == +RtcmMessageType::GAL_SSR_COMB_CORR)
		{
			...
			if (acsConfig.output_downloaded_rtcm_json)
				outputSsrEphToJson(ssrEph, Sat, acsConfig.downloaded_rtcm_json_filename);
			...
			||message_number == +RtcmMessageType::GAL_SSR_COMB_CORR)
		{
			...
			if (acsConfig.output_downloaded_rtcm_json)
				outputSsrClkToJson(ssrClk, Sat, acsConfig.downloaded_rtcm_json_filename);
...
void initSsrOut()
{
	...
	for (auto& [satId, entry] : nav.satNavMap)
	{
	...
	GTime 				time)			///< Time of current epoch
{
	...
	for (auto& sat : sats)
	{
		...
		GTime teph		= time;
		GTime time_1	= time;
		GTime time_2	= time + ssrOut.ssrEph.udi;
		bool pass;
		...
		{
			PhaseCenterData* satPCD = findAntenna(obs.Sat.id(), time, nav);
		...
		pass = satpos(trace, time_1, teph, obs, E_Ephemeris::BROADCAST, 					E_OffsetType::APC, nav, pcoMap_ptr);
		if (pass == true)
		{
			...
			ssrOut.ssrEph.broadcast_1		= obs.rSat;
			ssrOut.ssrEph.broadcastVel_1	= obs.satVel;
			ssrOut.ssrEph.isBroadcastSet_1	= true;
			ssrOut.ssrEph.iode				= obs.iode;
		}

		pass = satpos(trace, time_1, teph, obs, acsConfig.ssrOpts.ssr_ephemeris_source,	E_OffsetType::APC, nav, pcoMap_ptr);
		if (pass)
		{
			ssrOut.ssrEph.precise_1			= obs.rSat;
			ssrOut.ssrEph.isPreciseSet_1	= true;
		}
		...
		pass = satpos(trace, time_2, teph, obs, E_Ephemeris::BROADCAST,					E_OffsetType::APC, nav, pcoMap_ptr);
		if (pass)
		{
			ssrOut.ssrEph.broadcast_2		= obs.rSat;
			ssrOut.ssrEph.broadcastVel_2	= obs.satVel;
			ssrOut.ssrEph.isBroadcastSet_2	= true;

			assert(ssrOut.ssrEph.iode == obs.iode);
		}

		pass = satpos(trace, time_2, teph, obs, acsConfig.ssrOpts.ssr_ephemeris_source,	E_OffsetType::APC, nav, pcoMap_ptr);
		if (pass == true)
		{
			ssrOut.ssrEph.precise_2			= obs.rSat;
			ssrOut.ssrEph.isPreciseSet_2	= true;
		}
	}
	...
	for (auto& [satId, entry] : nav.satNavMap)
	{
		...
		if (ssrOut.epochsSinceUpdate >= ssrOut.udiNumEpochs)
		{
				...
				&&ssrOut.numObs > 0)
			{
				...
				if (acsConfig.output_uploaded_rtcm_json)
					outputSsrClkToJson(ssrClk, Sat, acsConfig.uploaded_rtcm_json_filename);
			}
			...
			SSREphOut& ssrEph = ssrOut.ssrEph;		//todo aaron, remove ssr from navigation.hpp
			if	( ssrEph.isPreciseSet_1
				&&ssrEph.isPreciseSet_2
				&&ssrEph.isBroadcastSet_1
				&&ssrEph.isBroadcastSet_2
				&&ssrOut.numObs > 0)
			{
				Vector3d	diffEcef_1			= ssrEph.broadcast_1 - ssrEph.precise_1;
				Vector3d	diffEcef_2			= ssrEph.broadcast_2 - ssrEph.precise_2;
				
				Vector3d	diffRac_1			= ecef2rac(ssrEph.broadcast_1, 	ssrEph.broadcastVel_1)	* diffEcef_1;
				Vector3d	diffRac_2			= ecef2rac(ssrEph.broadcast_2, 	ssrEph.broadcastVel_2)	* diffEcef_2;
				
				Vector3d	aveDiffRac			= (diffRac_1 + diffRac_2) / 2;
				Vector3d	dDiffRac			= (diffRac_2 - diffRac_1) / ssrEph.udi;

				int udiIndex					= SSRDecoder::getUdiIndex((int)round(ssrEph.udi));
				if (udiIndex == -1)
					BOOST_LOG_TRIVIAL(error) << "Error: ssrEph.udi is not valid (" << ssrEph.udi << ").";

				ssrEph.deph						= aveDiffRac; 
				ssrEph.ddeph					= dDiffRac;
				...
				if (acsConfig.output_uploaded_rtcm_json)
					outputSsrEphToJson(ssrEph, Sat, acsConfig.uploaded_rtcm_json_filename);
			}
			ssrEph.isPreciseSet_1 		= false;
			ssrEph.isBroadcastSet_1 	= false;
