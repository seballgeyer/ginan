	...
	Network&				net)
{
	...
	{
		createNewTraceFile(net.id,		logptime,	acsConfig.downloaded_rtcm_json_filename,				acsConfig.downloaded_rtcm_json_filename,								false,	false);
	}

	if	(acsConfig.output_uploaded_rtcm_json)
	{
		createNewTraceFile(net.id,		logptime,	acsConfig.uploaded_rtcm_json_filename,					acsConfig.uploaded_rtcm_json_filename,									false,	false);
	}
...
#			endif
		...
		if (acsConfig.ssrOpts.calculate_ssr)
		{
			...
			calcSsrCorrections(netTrace, KF_ARcopy, sats, tsync);
	...
	char**	argv)
{
	...
	for (auto& directory : {
								...
								acsConfig.uploaded_rtcm_json_directory,
								acsConfig.downloaded_rtcm_json_directory
							})
	{
