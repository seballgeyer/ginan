...
def get_sisre(sp3_a:_pd.DataFrame,sp3_b:_pd.DataFrame,clk_a:_pd.DataFrame,clk_b:_pd.DataFrame)->_pd.DataFrame:
    ...
    rac_dt = _np.unique(rac.index.droplevel(1).values)
    ...
    common_dt = _np.intersect1d(rac_dt,_np.intersect1d(clk_a.index.levels[0],clk_b.index.levels[0]))
    ...
    common_svs = _np.intersect1d(clk_a_unst.columns,clk_b_unst.columns)
